      ...
      <main class="hs-bm-panel">
        ...
        <section class="hs-bm-overview" aria-labelledby="black-market-overview-title">
          ...
          <dl class="hs-bm-summary" aria-label="剩余商品购买总计">
            <div><dt>未兑换商品总价</dt><dd>{{ formatNumber(marketPlan.totalPrice) }}<small> 血石</small></dd></div>
            <div><dt>扣除已有血石后还需</dt><dd>{{ formatNumber(marketPlan.remainingBloodstones) }}<small> 血石</small></dd></div>
          ...
          <p class="hs-bm-deadline" :class="marketPlan.canComplete ? 'is-possible' : 'is-impossible'">
            ...
            <span v-else>{{ marketPlan.canComplete ? '按当前进度可买齐' : '按当前进度无法买齐' }}；预计 {{ formatPlanDate(marketPlan.completionDate) }} 买齐</span>
          </p>
            ...
            <table class="hs-bm-table">
              ...
              <tbody>
                <tr v-for="item in marketItems" :key="item.id" :class="{ 'is-exchanged': item.isExchanged }">
                  <th scope="row">{{ item.name }}<small v-if="item.quantity > 1">×{{ item.quantity }} · 剩 {{ item.remainingQuantity }} 包</small></th>
...
import { calculateBlackMarketItem, calculateBlackMarketPlan, getBlackMarketAdvice } from '../utils/blackMarket.js'

const STORAGE_KEY = 'hs:black-market'
const now = ref(new Date())
let clockTimer
onMounted(() => { clockTimer = window.setInterval(() => { now.value = new Date() }, 60_000) })
...
.hs-bm-summary{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px;margin:0}.hs-bm-summary>div{min-width:0;padding:14px 16px;border:1px solid rgba(217,172,89,.38);border-radius:8px;background:linear-gradient(135deg,rgba(106,71,43,.3),rgba(18,15,19,.62))}.hs-bm-summary dt{color:var(--bm-muted);font-size:12px}.hs-bm-summary dd{margin:7px 0 0;color:#fff0c4;font:700 25px Georgia,'Microsoft YaHei',serif;font-variant-numeric:tabular-nums}.hs-bm-summary dd small{color:#dfc58e;font-size:12px}.hs-bm-summary-note{margin:8px 0 16px;color:var(--bm-muted);font-size:12px;line-height:1.5}
...
@media (max-width:760px){.hs-bm-summary{grid-template-columns:1fr;gap:8px}.hs-bm-summary>div{padding:10px 13px}.hs-bm-summary dd{margin-top:3px;font-size:21px}}
