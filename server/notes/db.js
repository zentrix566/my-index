...
CREATE TABLE IF NOT EXISTS idea_notes (
  ...
  CHECK (category IN ('idea', 'vibe_coding', 'memo')),
...
async function createPgDriver() {
  ...
  await pool.query("ALTER TABLE idea_notes ADD CONSTRAINT idea_notes_category_check CHECK (category IN ('idea', 'vibe_coding', 'memo'))")
