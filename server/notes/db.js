...
function buildLibraryIndexes() {
  return `
CREATE INDEX IF NOT EXISTS idx_idea_notes_user_revisit ON idea_notes(user_id, revisit_at);
CREATE INDEX IF NOT EXISTS idx_idea_notes_user_topic ON idea_notes(user_id, topic);
`
}
...
async function createSqliteDriver() {
  ...
  let columns = database.prepare('PRAGMA table_info(idea_notes)').all()
  const addColumn = (name, sql) => {
    if (columns.some((column) => column.name === name)) return
    database.exec(`ALTER TABLE idea_notes ADD COLUMN ${sql}`)
    columns = database.prepare('PRAGMA table_info(idea_notes)').all()
  }
...
async function createPgDriver() {
  ...
  await pool.query("ALTER TABLE idea_notes ADD COLUMN IF NOT EXISTS tags TEXT NOT NULL DEFAULT '[]'")
  ...
  await pool.query(buildLibraryIndexes())
