...
- **灵感收集**：按月或跨全部时间保存想法、Vibe Coding、备忘和梦境，支持主题归档、置顶与待回看日期提醒、标签 / 关键词 / 分类筛选、倒序时间轴分页、逐日记录节奏、快速记录与随机重读、单条分享图片和 JSON 导出；详情页可一键寻找相近记录或调用 AI 生成项目雏形，把零散想法延伸为下一步方向；记录详情可添加图片附件，并可连同单条分享图片导出，图片私有保存于 OSS，仅登录账号本人可查看；数据独立存于 `data/notes.local.db`（生产为 PostgreSQL 库 `zentrix_notes`）。
