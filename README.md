...
- **炉石成就追踪增强**：支持本地规则型冲刺推荐、AI 建议核心/全部版本范围切换、个人中心成就图合成分享，以及按职业浏览时使用硬核模式纳入全部版本；按职业浏览与我的成就已取消版本分组（跨版本平铺总览，版本按发布时间从新到旧），"我的成就 - 按职业"与"待完成清单"采用一致的分组（一次性成就 / 累计-次数 / 累计-点数，组内剩余从低到高、已完成排在组尾，分组默认收起可展开），硬核模式为视图切换后的全站总开关。
