...
- **日程管理（Todo）**：轻量任务管理，任务支持待办、进行中、已延期、等待中、已完成、已取消六态切换，复用全站统一登录；任务按「今日待办 / 今日已完成」视图组织，支持自定义分组（默认 工作/学习/生活，可增删）、优先级、日历月/周视图（周视图默认、周一为起始、含热力统计）与日程 AI 分析（今日/本周/本月，由 DeepSeek 生成）；今日已完成支持一键导出分享图片；数据独立存于 `data/todo.local.db`（生产为独立 PostgreSQL 库 `zentrix_todo`），与主站账号体系完全隔离。
