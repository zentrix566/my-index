...
- **暗月宝藏模拟器**（`/hearthstone/wheel`）：当前「比格沃斯先生」奖池支持模拟抽取 10 项奖励、查看首抽概率与宝珠花费；可切换历史奖池并保留各期记录，快速抽奖在获得当期大奖后停止。异画奖励暂用普通卡面预览，详情中会明确标注。
- **炉石成就追踪增强**：支持本地规则型冲刺推荐、个人中心成就图合成分享，以及按职业浏览时使用硬核模式纳入全部版本；按职业浏览与我的成就已取消版本分组（跨版本平铺总览，版本按发布时间从新到旧），"我的成就 - 按职业"与"待完成清单"采用一致的分组（一次性成就 / 累计-次数 / 累计-点数，组内剩余从低到高、已完成排在组尾，分组默认收起可展开），硬核模式与三个视图切换同排显示并对全部视图生效。支持导入采集器导出的 `achievements.json` 一键同步游戏内成就进度（经 `achievement-id-map.json` 将游戏数字 ID 映射为站内成就，预览确认后合并，只推进不回退）。
- **炉石外观收藏**：集中管理英雄皮肤、幸运币和卡背，支持按职业浏览、名称搜索、独立进度、已拥有优先及未拥有黑白显示；登录后收藏状态持久化到独立明细表，外观图片使用英文目录存储于 OSS，并提供下载、客户端提取、清单检查与增量上传脚本。收藏分页支持首页、末页与跳至指定页，每页显示个数可按外观类型分别设置并自动记忆，暂缺图片的外观可标记隐藏避免裂图；列表使用 WebP 缩略图并采用占位加淡入的加载方式，图片加载超时自动兜底防永久转圈，详情弹窗仍显示原图。收藏采集工具（`tools/hs-cosmetics-collector/`，C# Windows 图形界面程序）读取游戏内存生成 `cosmetics.json` 与 `achievements.json`，网站端选择文件即可预览并导入已拥有外观与成就进度，导入时自动剔除目录重建后遗留的旧 ID；导入后发现标记有误时，可用「批量设为未拥有」一键清空当前类型的已拥有项并带二次确认，便于重新采集导入。收藏页与卡牌查询页的大体量目录数据（皮肤/幸运币/卡背/卡牌库）按需加载：卡牌库（`cards-db.json`）托管于阿里云 OSS（`/hearthstone-data/` 前缀），更新数值后重新上传即可、无需重新构建部署；其余收藏目录放在 `public/hearthstone/` 按需加载并被浏览器长效缓存，页面脚本体积缩减 95% 以上，发版不会让用户重复下载未变化的数据。采集器下载包文件名带版本号（`hs-cosmetics-collector-vX.Y.Z.zip`），可用 `tools/hs-cosmetics-collector/build-release.ps1` 一键构建并生成带版本号的发布包与 SHA256（本机无 .NET SDK 时脚本自动下载 Roslyn 编译器，构建后请将生成的 zip 提交到仓库以更新下载）。
- **炉石卡牌图库与检索数据**：基于暴雪国服官方 API 全量重建卡牌图（按「卡名_卡id」命名消除同名歧义，全图 png / 缩略图 jpg 按真实格式上传云端），同步已公开的活动与新版本卡牌；少数官网接口与游戏客户端不一致的数值以游戏内实测为准。生成 `cards-db.json` 供图片上传和未来卡片检索使用；卡组代码解析统一读取由 HearthstoneJSON 最新 zhCN 全量库生成的 `dbfid-cardnames.json`，覆盖可收藏卡、衍生卡、英雄与历史卡牌。成就搜索（含待完成清单）现已跨全版本（含硬核与更多版本），并支持按成就内容匹配（阶段描述、关联卡牌）而不只是标题。
...
- **项目快捷导航**：顶部工作项目下拉可直达三个工作项目，个人项目下拉聚焦炉石传说成就档案、日常管理和灵感收集，其余工具统一在完整个人项目列表中查看。
- **抵御心魔**：记录每日抵御心魔（扛住/破防）与正能量状态，支持计时挑战、日历回看、AI 复盘分析（今天/上周/本月/指定天/时间段）、成就系统与个性化配置（心魔与正能量种类可拖拽排序、归档）。
- **日程管理（Todo）**：轻量任务管理，任务支持待办、进行中、已延期、等待中、已完成、已取消六态切换，复用全站统一登录；今日待办优先展示今天计划，随后提醒历史逾期任务，并支持快速改到今天、明天或补记完成日期，改期后保留原计划与历史记录；新增、修改或删除后导航角标实时刷新；支持自定义分组（默认 工作/学习/生活，可增删，并记住上次新建任务使用的分组）、优先级、日历月/周视图（周视图默认、周一为起始、含热力统计；每个任务只在当前计划日或实际完成日显示一次，延期完成以不同颜色区分，点开当天可查看计划完成日期与逾期天数）与日程 AI 分析（今日/本周/本月，由 DeepSeek 生成）；今日已完成支持一键导出分享图片；数据独立存于 `data/todo.local.db`（生产为独立 PostgreSQL 库 `zentrix_todo`），与主站账号体系完全隔离。
- **灵感收集**：按月或跨全部时间保存想法、Vibe Coding、备忘和梦境，支持置顶、关键词与分类筛选，以及灵感卡片、日期归档和记录节奏三种查看方式；记录按展示时间排序，梦境可自行设置记录日期和时间，补记或未来记录会按所选时间展示与归档；记录节奏按日展示数量并可进入当天归档。新建内容会自动保存本机草稿，保存成功后及时清理，编辑时点击弹窗外部不会意外关闭。支持快速记录、详情字数与范围统计、AI 月度分析、加入 Todo 收集箱、单条分享图片和 JSON 导出；记录详情可添加图片附件，并可连同分享图片导出，图片私有保存于 OSS，仅登录账号本人可以查看；数据独立存于 `data/notes.local.db`（生产为 PostgreSQL 库 `zentrix_notes`）。
...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 北京地铁 · 站站距离（`/subway`）：覆盖北京全部运营线路与车站，支持站站最短距离规划、手动指定换乘线路，以及按线路浏览逐站距离；纯本地计算，无需联网。
  - 历史时间线（/history）：中国历代王朝与重大事件的可视化时间线，支持按朝代/皇帝跳转、人物/事件/典故搜索；世界历史时间线见 /world-history。
  - 炉石传说成就档案（`/hearthstone`）：按扩展包或职业浏览炉石成就，支持进度保存、最多 10 项置顶追踪、成就分享卡片（已完成/进行中/置顶合集三种模式）、Excel/JSON 导入导出、卡组代码解析与关联卡牌原画查看；黑市血石计算器可录入实时价格和已有血石，展示相对首发价的涨跌、建议入手价（原价 9 折）、距每日 15 点变价的倒计时、入手建议与攒取天数，整组商品可填写已拥有份数只算还差的部分，并可标记已兑换商品。
  ...
  - 日程管理（`/todo`）：轻量待办与日程，含今日计划、逾期提醒、改期记录、补记完成、今日已完成视图、自定义分组（默认 工作/学习/生活，可增删）、优先级、日历月/周视图（任务只显示在当前计划日或实际完成日，延期完成以不同颜色区分，点开当天可查看计划完成日期与逾期天数；周视图默认、周一为起始）与日程 AI 分析、今日已完成图片导出。
  - 灵感收集（`/notes`）：按月或跨全部时间管理想法、Vibe Coding、备忘和梦境，支持置顶、按展示时间排序、梦境时间自定义、本机草稿保护、关键词与分类筛选、灵感卡片、日期归档、每日数量柱状图、快速记录、AI 月度分析、加入 Todo 收集箱、私有图片附件、单条分享图片与 JSON 导出。
  - 黄粱一梦（`/dream`）：输入当前年龄、期望寿命与人生野心，由 DeepSeek 流式生成按时间线铺陈的人间大梦；API Key 仅存服务端，前端经 `/api/dream` 代理调用，绝不暴露密钥。
  - 人物生平 · 纪年查询（`/biography`）：输入人名由 DeepSeek 生成生平年谱，API Key 仅存服务端，前端经 `/api/biography` 代理调用，绝不暴露密钥；查询结果本地缓存，重复查询可立即查看，最近查询记录支持单条删除或一键清空。
  - 炉石项目内功能：蛙生模拟器（`/hearthstone/frog`）和卡牌修改验收台（`/hearthstone/frog/review`）可从炉石模块进入，不在个人项目列表中单列。
- **全站更新日志**（`/changelog`）：主站、炉石、日程管理、灵感与其他工具共用一份日志数据，可按模块筛选；抵御心魔历史更新归入「其他」。
...
将卡牌数据库 `cards-db.json` 上传到 OSS（`hearthstone-data/` 前缀）。数据文件走 5 分钟缓存 + ETag，上传后无需重新构建部署，最多 5 分钟全量生效：
...
├── public/              # Vite 静态资源（采集工具下载包、外观收藏 JSON）
├── index.html            # Vite 入口
├── vite.config.js
├── Dockerfile            # 多阶段构建镜像
└── README.md
