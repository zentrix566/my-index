...
- **灵感收集**：按月或跨全部时间保存想法、Vibe Coding、备忘和梦境，支持置顶、关键词与分类筛选，以及灵感卡片、日期归档和记录节奏三种查看方式；记录节奏按日展示数量并可进入当天归档，更新时间同时显示星期几。支持快速记录、详情字数与范围统计、AI 月度分析、加入 Todo 收集箱、单条分享图片和 JSON 导出；记录详情可添加图片附件，并可连同分享图片导出，图片私有保存于 OSS，仅登录账号本人可查看；数据独立存于 `data/notes.local.db`（生产为 PostgreSQL 库 `zentrix_notes`）。
...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 灵感收集（`/notes`）：按月或跨全部时间管理想法、Vibe Coding、备忘和梦境，支持置顶、关键词与分类筛选、灵感卡片、日期归档、每日数量柱状图、快速记录、AI 月度分析、加入 Todo 收集箱、私有图片附件、单条分享图片与 JSON 导出。
