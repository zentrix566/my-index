...
- **日程管理（Todo）**：轻量任务管理，任务支持待办、进行中、已延期、等待中、已完成、已取消六态切换，复用全站统一登录；任务按「今日待办 / 今日已完成」视图组织，新增、修改或删除后导航角标实时刷新；支持自定义分组（默认 工作/学习/生活，可增删，并记住上次新建任务使用的分组）、优先级、日历月/周视图（周视图默认、周一为起始、含热力统计，已完成任务按实际完成日期归位，任务按状态稳定排序）与日程 AI 分析（今日/本周/本月，由 DeepSeek 生成）；今日已完成支持一键导出分享图片；数据独立存于 `data/todo.local.db`（生产为独立 PostgreSQL 库 `zentrix_todo`），与主站账号体系完全隔离。
- **灵感收集**：按月或跨全部时间保存想法、Vibe Coding、备忘和梦境，支持主题归档、置顶与待回看日期提醒、标签 / 关键词 / 分类筛选、倒序时间轴分页、逐日记录节奏、快速记录与随机重读、详情字数统计与本月、全部时间范围统计、单条分享图片和 JSON 导出；详情页可一键寻找相近记录或调用 AI 生成项目雏形，把零散想法延伸为下一步方向；记录详情可添加图片附件，并可连同单条分享图片导出，图片私有保存于 OSS，仅登录账号本人可查看；数据独立存于 `data/notes.local.db`（生产为 PostgreSQL 库 `zentrix_notes`）。
...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 日程管理（`/todo`）：轻量待办与日程，含今日待办/已完成视图、自定义分组（默认 工作/学习/生活，可增删）、优先级、日历月/周视图（已完成任务按实际完成日期显示，周视图默认、周一为起始）与日程 AI 分析、今日已完成图片导出。
  - 灵感收集（`/notes`）：按月或跨全部时间管理想法、Vibe Coding、备忘和梦境，支持主题归档、置顶与待回看日期、标签 / 主题 / 分类筛选、时间轴分页、逐日记录节奏、快速记录与随机重读、私有图片附件、可带附件的单条分享图片、JSON 导出、相近记录查找与 AI 项目雏形。
