...
- **灵感收集**：按月保存想法、Vibe Coding 和备忘，支持标签、关键词与分类筛选、倒序时间轴、逐日记录节奏、单条图片分享和 JSON 导出；数据独立存于 `data/notes.local.db`（生产为 PostgreSQL 库 `zentrix_notes`）。
...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 灵感收集（`/notes`）：按月管理想法、Vibe Coding 和备忘，支持标签、筛选、时间轴、逐日记录节奏、单条图片分享与 JSON 导出。
