...
- **日程管理（Todo）**：轻量任务管理，任务支持待办、进行中、已延期、等待中、已完成、已取消六态切换，复用全站统一登录；今日待办优先展示今天计划，随后提醒历史逾期任务，并支持快速改到今天、明天或补记完成日期，改期后保留原计划与历史记录；新增、修改或删除后导航角标实时刷新；支持自定义分组（默认 工作/学习/生活，可增删，并记住上次新建任务使用的分组）、优先级、日历月/周视图（周视图默认、周一为起始、含热力统计，已完成任务按实际完成日期归位，月、周和单日详情会显示逾期或延期完成天数，任务按状态稳定排序）与日程 AI 分析（今日/本周/本月，由 DeepSeek 生成）；今日已完成支持一键导出分享图片；数据独立存于 `data/todo.local.db`（生产为独立 PostgreSQL 库 `zentrix_todo`），与主站账号体系完全隔离。
...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 日程管理（`/todo`）：轻量待办与日程，含今日计划、逾期提醒、改期记录、补记完成、今日已完成视图、自定义分组（默认 工作/学习/生活，可增删）、优先级、日历月/周视图（已完成任务按实际完成日期显示，并标注逾期或延期完成天数，周视图默认、周一为起始）与日程 AI 分析、今日已完成图片导出。
