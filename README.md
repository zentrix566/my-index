...
- **个人作品**（`/projects`）：与工作项目共用项目索引，把自己写的小页面和工具收拢成卡片，点卡片直接进入：
  ...
  - 蛙生模拟器（`/hearthstone/frog`）：三张标准模式随从中有一张被蛙生悄悄动了手脚，玩家需揪出假牌；混淆类型可勾选（默认法力值、攻击、生命做 ±1 微调），仅收录标准模式随从，卡图走 OSS 反向代理，狂野版本调试完毕后敬请期待；另设「卡牌修改验收台」（`/hearthstone/frog/review`）对照原始卡与修改后卡、可显示补丁边界。
